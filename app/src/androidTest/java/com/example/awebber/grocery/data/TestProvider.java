...
public class TestProvider extends AndroidTestCase {
    ...
    public void deleteAllRecordsFromProvider() {
        ...
        assertEquals("Error: Records not deleted from Groceries table during delete", 0, cursor.getCount());
        cursor.close();
        ...
        assertEquals("Error: Records not deleted from Basic Description table during delete", 0, cursor.getCount());
        cursor.close();
    }
    ...
    public void testProviderRegistry() {
        ...
        // GroceryContentProvider class.
        ComponentName componentName = new ComponentName(mContext.getPackageName(),
                GroceryContentProvider.class.getName());
        try {
            ...
            assertEquals("Error: GroceryContentProvider registered with authority: " + providerInfo.authority +
                            " instead of authority: " + GroceryContract.CONTENT_AUTHORITY,
                    providerInfo.authority, GroceryContract.CONTENT_AUTHORITY);
        } catch (PackageManager.NameNotFoundException e) {
            // I guess the provider isn't registered correctly.
            assertTrue("Error:  GroceryContentProvider not registered at " + mContext.getPackageName(),
                    false);
        }
    }
    ...
    public void testGetType() {
        // content://com.example.awebber.grocery/groceries/
        String type = mContext.getContentResolver().getType(GroceryEntry.CONTENT_URI);
        // vnd.android.cursor.dir/com.example.awebber.grocery.app/grocery
        assertEquals("Error: the GroceryEntry CONTENT_URI should return GroceryEntry.CONTENT_TYPE",
                GroceryEntry.CONTENT_TYPE, type);

        // content://com.example.awebber.grocery/brands/
        type = mContext.getContentResolver().getType(BrandEntry.CONTENT_URI);
        // vnd.android.cursor.dir/com.example.awebber.grocery/brands
        assertEquals("Error: the BrandEntry CONTENT_URI should return BrandEntry.CONTENT_TYPE",
...
/*
        ...
        type = mContext.getContentResolver().getType(
                GroceryEntry.buildGroceriesBrand(testLocation));
...
/*
    ...
    static ContentValues[] createBulkInsertWeatherValues(long locationRowId) {
        ...
        for ( int i = 0; i < BULK_INSERT_RECORDS_TO_INSERT; i++, currentTestDate+= millisecondsInADay ) {
            ...
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_BRAND_LOC_KEY, locationRowId);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_BRAND_LOC_KEY, locationRowId);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_DATE, currentTestDate);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_DEGREES, 1.1);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_HUMIDITY, 1.2 + 0.01 * (float) i);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_PRESSURE, 1.3 - 0.01 * (float) i);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_MAX_TEMP, 75 + i);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_MIN_TEMP, 65 - i);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_SHORT_DESC, "Asteroids");
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_WIND_SPEED, 5.5 + 0.2 * (float) i);
            weatherValues.put(GroceryContract.GroceryEntry.COLUMN_WEATHER_ID, 321);
